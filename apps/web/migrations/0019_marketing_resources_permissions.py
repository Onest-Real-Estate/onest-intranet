"""Create and grant marketing resources administration permissions."""

from django.db import migrations

PERMISSION_NAMES = {
    "manage_marketing_resources": "Can manage marketing resources",
    "publish_marketing_resources": (
        "Can publish, schedule, and archive marketing resources"
    ),
    "download_marketing_sources": "Can download marketing source files",
}
...
def grant_permissions(apps, schema_editor):
    ...
    for group_name in MANAGE_GROUPS:
        ...
        group.permissions.add(permissions["manage_marketing_resources"])
    for group_name in PUBLISH_GROUPS:
        group, _ = Group.objects.get_or_create(name=group_name)
        group.permissions.add(permissions["publish_marketing_resources"])
    for group_name in SOURCE_GROUPS:
        group, _ = Group.objects.get_or_create(name=group_name)
        group.permissions.add(permissions["download_marketing_sources"])
...
class Migration(migrations.Migration):
    dependencies = [
        ("web", "0018_onboarding_tool_catalog_permission"),
    ]
