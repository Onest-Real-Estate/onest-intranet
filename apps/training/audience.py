...
contents published long before. That is the documented policy, and it is
why nothing here is cached against the content.
...
indexed subquery. Answering "can this person see these fifty contents"
...
def selectors_for(content: TrainingContent) -> QuerySet[TrainingAudience]:
    """The stored selectors for one content, targets already joined."""
    return TrainingAudience.objects.filter(content=content).select_related(
        "office", "user"
    )
...
def selector_q(context: AudienceContext) -> Q:
    """The union, expressed over :class:`TrainingAudience` rows.
...
def audience_q(context: AudienceContext) -> Q:
    """The union, as one ``Q`` over ``TrainingContent``.
    ...
    * **It is one indexed subquery** regardless of how many selectors exist,
    ...
    if not context.is_authenticated:
        return Q(pk__in=[])
    return Q(
        pk__in=TrainingAudience.objects.filter(selector_q(context)).values("content_id")
    )


def visible_contents(user, *, at=None, queryset=None) -> QuerySet[TrainingContent]:
    """Published, in-window training whose audience includes ``user``."""
    moment = at or timezone.now()
    base = TrainingContent.objects.all() if queryset is None else queryset
    return (
        base.published()
...
def visible_training_content(
    user, *, at=None, queryset=None
) -> QuerySet[TrainingContent]:
    return visible_contents(user, at=at, queryset=queryset)


def visible_to(user, content: TrainingContent, *, at=None) -> bool:
    ...
    if content.pk is None:
        return False
    return visible_contents(user, at=at).filter(pk=content.pk).exists()


def assert_visible(user, content: TrainingContent, *, at=None, reason: str) -> None:
    """``visible_to`` with the denial recorded. Use on direct-object access."""
    if visible_to(user, content, at=at):
        return
    log_event(
        ...
        target=AuditTarget(
            ...
            target_id=str(content.pk or ""),
            target_label=content.slug,
        ),
        outcome=AuditEvent.Outcome.DENIED,
        reason=reason,
    )
    raise PermissionDenied("That training item is not addressed to you.")


# --------------------------------------------------------------------------- #
# The other direction: who does this content reach?
# --------------------------------------------------------------------------- #


def recipients_for(content: TrainingContent, *, at=None) -> QuerySet[User]:
    """Every user the content's selectors reach, deduplicated.

    Used by notification fan-out. One queryset with a union filter rather than
    a loop over selectors, so a company-wide content is one query and each
    person appears once however many selectors caught them.
    """
    moment = at or timezone.now()
    selectors = list(selectors_for(content))
    if not selectors:
...
def _deny(actor, *, reason: str, detail: dict | None = None) -> None:
    log_event(
        "security.training.audience_denied",
        actor=actor_from_user(actor),
        target=AuditTarget(target_type=TrainingContent._meta.label_lower, target_id=""),
        outcome=AuditEvent.Outcome.DENIED,
        reason=reason,
        metadata=detail or {},
    )
...
def assert_can_target(actor, selectors) -> None:
    ...
    if not has_effective_permission(actor, MANAGE_PERMISSION):
        ...
        raise PermissionDenied("You cannot publish training content.")
    if not selectors:
...
def replace_audience(actor, content: TrainingContent, selectors) -> None:
    """Authorize, then make the stored selectors match exactly what was asked."""
    selectors = list(selectors)
    assert_can_target(actor, selectors)
    before = describe_audience(content)
    selectors_for(content).delete()
    ...
    log_event(
        ...
        after={"audience": describe_audience(content)},
    )
...
def describe_audience(content: TrainingContent) -> list[dict[str, Any]]:
    """camelCase summary of the selectors, in a stable order.

    Ordered by kind then label so two contents with the same audience
    ...
    rows = [
        ...
        for selector in selectors_for(content)
    ]
    return sorted(rows, key=lambda row: (order.get(row["kind"], 9), row["label"]))


def _selector_label(selector: TrainingAudience) -> str:
    if selector.kind == Kind.COMPANY:
...
def search_recipients(actor, query: str, *, limit: int = RECIPIENT_SEARCH_LIMIT):
    ...
    if not has_effective_permission(actor, MANAGE_PERMISSION):
        _deny(actor, reason="missing_permission")
        raise PermissionDenied("You cannot address training content.")
