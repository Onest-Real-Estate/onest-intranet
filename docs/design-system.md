...
- **Navigation:** keep the primary nav stable. The active item uses the sidebar accent surface, semibold type, a `text-primary` icon, and the gold left rail, and it also exposes `aria-current="page"`. Sidebar sections are declared in `HUB_NAV_GROUPS`; the uppercase group label is the only separation — no rules between groups. Sections without a backend show a "Soon" marker as plain muted type, never a pill: a rail of a dozen chips reads as a mockup rather than a product. Availability is explicit, not inferred from the URL — `resolveHubNav` reads the backend's `features` map (`apps/web/navigation.py`) and the user's `primaryOffice`, and an office-scoped item for a user with no office reads "No office" instead. The marker itself is `aria-hidden`; the sentence a screen reader gets comes from `aria-describedby`.
