...
import type { CompanyContractSignPageProps } from "@/types";

function fieldError(
  errors: CompanyContractSignPageProps["errors"],
  key: string,
): string | undefined {
...
export default function CompanyContractSign() {
  const { canSign, signingReady, recovery, disclosure, contract, ceremony, errors } =
    usePage<CompanyContractSignPageProps>().props;
  ...
  function startCeremony() {
    ...
    router.post(
      routes.agent_contract_company_sign(contract.publicId),
      {
  ...
  async function submitSignature() {
    if (!ceremony || !contract || completing) return;
    setCompleting(true);
    setCompleteError(null);
    try {
      const response = await fetch(
        routes.agent_contract_company_sign_complete(contract.publicId),
        {
          method: "POST",
      ...
      router.visit(
        data.workspaceUrl || routes.agent_contract_workspace(contract.publicId),
        { replace: true },
      );
    } catch {
  ...
  return (
    <>
      <Head title="Sign for the company" />
      <div className="grid max-w-3xl gap-8">
        <PageHeader
          title="Sign for the company"
          description="Acknowledge the disclosure, then apply the Company signature before the agent can sign."
          actions={
            contract ? (
              <Button type="button" variant="ghost" asChild>
                <Link href={routes.agent_contract_workspace(contract.publicId)}>
            ...
            <EmptyState
              ...
              actions={
                <Button variant="outline" asChild>
                  <Link
                    href={routes.agent_contract_workspace(contract?.publicId ?? "")}
                  >
                    Open workspace
                  </Link>
                </Button>
                ...
                <>
                  ...
                  <div className="grid gap-3">
                    ...
                    {!signingReady ? (
                      ...
                      >
                        You can still review the agreement from the contract workspace.
                      </Callout>
                    ) : null}
                  </div>
                </>
...
CompanyContractSign.layout = (page: React.ReactNode) => <HubLayout>{page}</HubLayout>;
