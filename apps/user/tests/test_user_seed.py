...
def test_seed_users_creates_demo_accounts_with_roles():
    ...
    assert len(report.created) == len(DEV_USER_TEMPLATES)
...
def test_seed_users_is_idempotent():
    ...
    assert len(first.created) == len(DEV_USER_TEMPLATES)
    assert second.created == []
    assert len(second.matched) == len(DEV_USER_TEMPLATES)
    assert second.assignments_created == []
    assert len(second.assignments_matched) == len(DEV_USER_TEMPLATES) - 1
