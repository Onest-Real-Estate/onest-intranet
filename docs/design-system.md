...
| Metric figure | `text-metric font-bold tracking-[-0.02em] tabular-nums` | Stat cards, via `MetricCard` |
...
- **Filters:** `FilterControls` is the one filter surface on a list page: a pill that names the region and carries the count of what is narrowing the list, the fields, and a reset that exists only while there is something to reset. The panel starts open — on an operational list the filters *are* the control surface, and a reader who cannot see the constraint blames the data — so collapsing is a choice, not the default. Pass `collapsible={false}` where the fields are the page's primary control, as on a report. A page has one filter surface, never a visible row plus a second disclosure below it.
...
All interactive controls need a visible focus ring, a minimum practical target of 36 px (44 px on touch-heavy screens), a disabled state, and clear hover/pressed feedback. Animations should generally stay between 120–200 ms. `app.css` carries the global `prefers-reduced-motion` path so individual components do not repeat it, and that path **reduces rather than disables**: transitions keep easing but only over colour, border, opacity, and shadow, so hover, focus, and press still read as feedback while transforms land instantly. Indeterminate progress is exempt and keeps looping more slowly — a stopped spinner does not read as reduced motion, it reads as hung. A loop that seeks attention rather than reporting progress (`animate-ping`) stops.
