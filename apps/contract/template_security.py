...
Structural safety for PDF templates that DocuSeal Builder will annotate:
...
- Fillable field names come from DocuSeal after builder sync — blank PDFs are OK.
"""
...
# Hub commercial / party / office sources authors may map DocuSeal fields onto.
...
def infer_template_format(*, filename: str, media_type: str) -> str:
    ...
    raise ValidationError(
        {"source_document": "Upload a PDF contract template for DocuSeal."}
    )
...
def inspect_template(
    *,
    ...
    return TemplateInspection(
        ...
        # Field names are synced from DocuSeal after builder placement.
        placeholder_keys=(),
    )
...
def seed_merge_schema_from_placeholders(
    ...
    """Build a starter allowlist from DocuSeal merge field names."""
    return [
        {
            "key": key,
            "label": key,
    ...
    if unknown_placeholders or missing_placeholders:
        ...
        if unknown_placeholders:
            errors["source_document"] = [
                "The DocuSeal template contains unmapped fields: "
                + ", ".join(unknown_placeholders)
            ]
        if missing_placeholders:
            errors.setdefault("merge_schema", []).append(
                "The merge schema declares fields missing from the DocuSeal "
