"""Forms for the documents workspace."""
...
from apps.documents.taxonomy import normalize_jurisdiction_codes
...
class DocumentVersionForm(forms.ModelForm):
    ...
    family_key = forms.SlugField(
        required=False,
        label=_("Family key"),
        help_text=_("Stable identity. Generated from the name when left empty."),
    )
    ...
    def __init__(self, *args: Any, actor: User, **kwargs: Any):
        ...
        category_field.queryset = DocumentCategory.objects.filter(
        ...
        if self.instance.pk:
            ...
            self.initial["owner_office"] = self.instance.owner_office.pk
            self.initial["family_key"] = self.instance.family.key
...
class DocumentTransitionForm(forms.Form):
    action = forms.ChoiceField(choices=[(value, value) for value in TRANSITIONS])
    expected_version = forms.CharField(required=False)


class DocumentDuplicateForm(forms.Form):
    expected_version = forms.CharField(required=False)
