"""Documents workspace HTTP surface."""

from __future__ import annotations

from typing import Any, cast

from django.core.exceptions import ValidationError
from django.http import HttpRequest, HttpResponse, JsonResponse
from django.shortcuts import get_object_or_404, redirect
from django.views.decorators.http import require_GET, require_POST
from inertia import inertia, render

from apps.audit.service import AuditTarget, actor_from_user, log_event
from apps.documents.administration import (
    PAGE_SIZE,
    StaleDocumentVersion,
    TransitionRefused,
    WorkspaceFilters,
    admin_row,
    apply_workspace_filters,
    audience_choice_payload,
    capabilities,
    category_options,
    create_document,
    detail_payload,
    document_version_token,
    duplicate_version,
    manageable_queryset,
    order_for_workspace,
    preview_payload,
    publishable_office_queryset,
    transition,
    update_document,
)
from apps.documents.audience import search_recipients
from apps.documents.forms import (
    DocumentDuplicateForm,
    DocumentTransitionForm,
    DocumentVersionForm,
)
from apps.documents.media_service import (
    admin_file_payload,
    admin_files_payload,
    allowed_matrix_payload,
    assert_admin_readable,
    assert_can_manage_media,
    remove_file,
    reorder_files,
    replace_file,
    stream_file,
    upload_document,
)
from apps.documents.models import DocumentFile, DocumentVersion
from apps.user.models import Office, User
from apps.user.services.role_assignments import has_effective_permission
from apps.web.authorization import enforce_policy
from apps.web.contracts import empty_validation_errors, list_response, validation_errors

INDEX_PAGE = "DocumentsAdministration"
WORKSPACE_PAGE = "DocumentsWorkspace"

MANAGE_PERMISSION = "web.manage_documents"

_SHEET_DRAFT_FIELDS: tuple[str, ...] = (
    "owner_office",
    "family_key",
    "name",
    "description",
    "category",
    "effective_at",
    "expires_at",
    "display_order",
)

_SHEET_DRAFT_LISTS: tuple[str, ...] = (
    "audience_roles",
    "audience_regions",
    "audience_offices",
    "audience_users",
)


def _require_manage(actor: User) -> None:
    from django.core.exceptions import PermissionDenied

    if not has_effective_permission(actor, MANAGE_PERMISSION):
        raise PermissionDenied("You cannot manage documents.")


def _page_param(request: HttpRequest) -> int:
    try:
        return max(1, int(request.GET.get("page", "1")))
    except ValueError:
        return 1


def _target(request: HttpRequest, document_id: int) -> DocumentVersion:
    actor = cast(User, request.user)
    return get_object_or_404(manageable_queryset(actor), pk=document_id)


def _sheet_draft(params) -> dict[str, Any]:
    draft: dict[str, Any] = {
        name: value for name in _SHEET_DRAFT_FIELDS if (value := params.get(name, ""))
    }
    for name in _SHEET_DRAFT_LISTS:
        values = params.getlist(name)
        if values:
            draft[name] = values
    if params.get("audience_company") in {"on", "true", "True"}:
        draft["audience_company"] = "on"
    return draft


def index_props(
    actor: User,
    *,
    params,
    page: int = 1,
    create_sheet: dict[str, Any] | None = None,
    errors: dict | None = None,
) -> dict[str, Any]:
    from apps.documents.models import DocumentCategory

    known_categories = list(DocumentCategory.objects.values_list("code", flat=True))
    filters = WorkspaceFilters.from_params(params, known_categories=known_categories)
    rows = order_for_workspace(
        apply_workspace_filters(manageable_queryset(actor), filters)
    )
    total = rows.count()
    total_pages = max(1, (total + PAGE_SIZE - 1) // PAGE_SIZE)
    current = min(max(page, 1), total_pages)
    start = (current - 1) * PAGE_SIZE
    items = [admin_row(row) for row in rows[start : start + PAGE_SIZE]]
    offices = [
        {"value": office.pk, "label": office.name}
        for office in publishable_office_queryset(actor)
    ]
    audience = audience_choice_payload(actor)
    caps = capabilities(actor).payload()
    return {
        "documents": list_response(
            items,
            page=current,
            page_size=PAGE_SIZE,
            total_items=total,
            filters=filters.as_payload(),
            sort_key="updatedAt",
            sort_direction="desc",
        ),
        "filterOptions": {
            "categories": category_options(include_codes=(filters.category,)),
            "offices": offices,
        },
        "createOptions": {
            "offices": offices,
            "categories": category_options(),
            "audience": audience,
        },
        "createSheet": create_sheet,
        "capabilities": caps,
        "errors": errors or empty_validation_errors(),
    }


@enforce_policy("operations_admin_documents")
@require_GET
@inertia(INDEX_PAGE)
def documents_administration_index(request: HttpRequest):
    actor = cast(User, request.user)
    _require_manage(actor)
    opening = request.GET.get("create") == "1"
    return index_props(
        actor,
        params=request.GET,
        page=_page_param(request),
        create_sheet={"open": True, "draft": {}} if opening else None,
    )


def _render_index_with_sheet_errors(
    request: HttpRequest, *, errors: dict
) -> HttpResponse:
    actor = cast(User, request.user)
    response = render(
        request,
        INDEX_PAGE,
        index_props(
            actor,
            params=request.POST,
            create_sheet={"open": True, "draft": _sheet_draft(request.POST)},
            errors=errors,
        ),
    )
    response.status_code = 422
    return response


def _preview_office(actor: User, raw: str) -> Office | None:
    if not raw.isdigit():
        return None
    return publishable_office_queryset(actor).filter(pk=int(raw)).first()


def workspace_props(
    actor: User,
    *,
    document: DocumentVersion | None = None,
    errors: dict | None = None,
    posted=None,
    preview_office: str = "",
    preview_role: str = "",
) -> dict[str, Any]:
    office = _preview_office(actor, preview_office) if document else None
    role = (
        preview_role
        if preview_role
        in {option["value"] for option in audience_choice_payload(actor)["roles"]}
        else ""
    )
    return {
        "document": detail_payload(document, actor=actor) if document else None,
        "officeOptions": [
            {"value": office_row.pk, "label": office_row.name}
            for office_row in publishable_office_queryset(actor)
        ],
        "categoryOptions": category_options(
            include_codes=(
                (document.category.code,) if document and document.category else ()
            )
        ),
        "audienceOptions": audience_choice_payload(actor),
        "capabilities": capabilities(actor).payload(),
        "preview": (
            {
                **preview_payload(document, actor=actor, office=office, role_code=role),
                "roleCode": role,
                "officeId": office.pk if office else None,
            }
            if document
            else None
        ),
        "errors": errors or empty_validation_errors(),
        "posted": _posted_payload(posted),
    }


def _posted_payload(posted) -> dict[str, list[str]] | None:
    if posted is None:
        return None
    return {key: posted.getlist(key) for key in posted}


@enforce_policy("document_admin_new")
@require_GET
@inertia(WORKSPACE_PAGE)
def document_admin_new(request: HttpRequest):
    actor = cast(User, request.user)
    _require_manage(actor)
    return workspace_props(actor)


@enforce_policy("document_admin_edit")
@require_GET
@inertia(WORKSPACE_PAGE)
def document_admin_edit(request: HttpRequest, document_id: int):
    actor = cast(User, request.user)
    _require_manage(actor)
    return workspace_props(
        actor,
        document=_target(request, document_id),
        preview_office=request.GET.get("previewOffice", "").strip(),
        preview_role=request.GET.get("previewRole", "").strip(),
    )


def _render_workspace(
    request: HttpRequest,
    *,
    document: DocumentVersion | None,
    errors: dict,
    status: int,
) -> HttpResponse:
    actor = cast(User, request.user)
    response = render(
        request,
        WORKSPACE_PAGE,
        workspace_props(
            actor,
            document=document,
            errors=errors,
            posted=request.POST,
        ),
    )
    response.status_code = status
    return response


def _validation_payload(exc: ValidationError) -> dict[str, Any]:
    if hasattr(exc, "message_dict"):
        data = exc.message_dict
        return {
            "fields": {
                field: [str(message) for message in messages]
                for field, messages in data.items()
                if field != "__all__"
            },
            "form": [str(message) for message in data.get("__all__", [])],
        }
    return {"fields": {}, "form": [str(message) for message in exc.messages]}


def _submit(request: HttpRequest, document: DocumentVersion | None) -> HttpResponse:
    actor = cast(User, request.user)
    from_sheet = document is None and request.POST.get("context") == "sheet"
    form = DocumentVersionForm(
        request.POST, instance=document or DocumentVersion(), actor=actor
    )
    if not form.is_valid():
        errors = validation_errors(form)
        if from_sheet:
            return _render_index_with_sheet_errors(request, errors=errors)
        return _render_workspace(request, document=document, errors=errors, status=422)
    try:
        if document is None:
            saved = create_document(
                actor=actor,
                office=form.cleaned_data["owner_office"],
                cleaned=form.field_values,
                selectors=form.selectors,
                family_key=form.cleaned_data.get("family_key") or "",
            )
        else:
            saved = update_document(
                actor=actor,
                version=document,
                cleaned=form.field_values,
                selectors=form.selectors,
                expected_version=form.cleaned_data.get("expected_version", ""),
            )
    except StaleDocumentVersion as exc:
        return _render_workspace(
            request,
            document=DocumentVersion.objects.get(pk=document.pk) if document else None,
            errors={"fields": {}, "form": [exc.message]},
            status=409,
        )
    except ValidationError as exc:
        errors = _validation_payload(exc)
        if from_sheet:
            return _render_index_with_sheet_errors(request, errors=errors)
        return _render_workspace(request, document=document, errors=errors, status=422)
    return redirect("document_admin_edit", document_id=saved.pk)


@enforce_policy("document_admin_create")
@require_POST
def document_admin_create(request: HttpRequest):
    _require_manage(cast(User, request.user))
    return _submit(request, None)


@enforce_policy("document_admin_update")
@require_POST
def document_admin_update(request: HttpRequest, document_id: int):
    _require_manage(cast(User, request.user))
    return _submit(request, _target(request, document_id))


def _lifecycle_failure(
    request: HttpRequest, document: DocumentVersion, message: str, status: int
) -> HttpResponse:
    actor = cast(User, request.user)
    response = render(
        request,
        WORKSPACE_PAGE,
        workspace_props(
            actor,
            document=DocumentVersion.objects.get(pk=document.pk),
            errors={"fields": {}, "form": [message]},
        ),
    )
    response.status_code = status
    return response


@enforce_policy("document_admin_lifecycle")
@require_POST
def document_admin_lifecycle(request: HttpRequest, document_id: int):
    actor = cast(User, request.user)
    _require_manage(actor)
    document = _target(request, document_id)
    form = DocumentTransitionForm(request.POST)
    if not form.is_valid():
        return _lifecycle_failure(
            request, document, "That is not a document action.", 422
        )
    try:
        transition(
            actor=actor,
            version=document,
            action=form.cleaned_data["action"],
            expected_version=form.cleaned_data.get("expected_version", ""),
        )
    except StaleDocumentVersion as exc:
        return _lifecycle_failure(request, document, exc.message, 409)
    except TransitionRefused as exc:
        return _lifecycle_failure(request, document, exc.message, 422)
    except ValidationError as exc:
        response = render(
            request,
            WORKSPACE_PAGE,
            workspace_props(
                actor,
                document=DocumentVersion.objects.get(pk=document.pk),
                errors=_validation_payload(exc),
            ),
        )
        response.status_code = 422
        return response
    return redirect("document_admin_edit", document_id=document.pk)


@enforce_policy("document_admin_duplicate")
@require_POST
def document_admin_duplicate(request: HttpRequest, document_id: int):
    actor = cast(User, request.user)
    _require_manage(actor)
    document = _target(request, document_id)
    form = DocumentDuplicateForm(request.POST)
    if not form.is_valid():
        return _lifecycle_failure(request, document, "Send the current version.", 422)
    try:
        draft = duplicate_version(
            actor=actor,
            version=document,
            expected_version=form.cleaned_data.get("expected_version", ""),
        )
    except StaleDocumentVersion as exc:
        return _lifecycle_failure(request, document, exc.message, 409)
    except ValidationError as exc:
        return _lifecycle_failure(
            request, document, "; ".join(str(m) for m in exc.messages), 422
        )
    return redirect("document_admin_edit", document_id=draft.pk)


@enforce_policy("document_admin_recipient_search")
@require_GET
def document_admin_recipient_search(request: HttpRequest):
    actor = cast(User, request.user)
    return JsonResponse({"results": search_recipients(actor, request.GET.get("q", ""))})


def _managed_document(actor: User, document_id: int) -> DocumentVersion:
    document = get_object_or_404(
        DocumentVersion.objects.select_related(
            "family", "family__owner_office", "category"
        ),
        pk=document_id,
    )
    assert_can_manage_media(actor, document)
    return document


@enforce_policy("document_admin_media")
@require_GET
@inertia("DocumentsMediaManager")
def document_admin_media(request: HttpRequest, document_id: int):
    actor = cast(User, request.user)
    document = _managed_document(actor, document_id)
    return {
        "document": {
            "id": document.pk,
            "name": document.name,
            "status": document.status,
            "version": document_version_token(document),
        },
        "files": admin_files_payload(document, actor=actor),
        "limits": allowed_matrix_payload(),
        "capabilities": capabilities(actor).payload(),
        "validation": empty_validation_errors(),
    }


def _media_error(exc: ValidationError, status: int = 422) -> JsonResponse:
    if hasattr(exc, "message_dict"):
        data = exc.message_dict
        return JsonResponse(
            {
                "validation": {
                    "fields": {
                        field: [str(message) for message in messages]
                        for field, messages in data.items()
                        if field != "__all__"
                    },
                    "form": [str(message) for message in data.get("__all__", [])],
                }
            },
            status=status,
        )
    return JsonResponse(
        {
            "validation": {
                "fields": {},
                "form": [str(message) for message in exc.messages],
            }
        },
        status=status,
    )


@enforce_policy("document_admin_media_upload")
@require_POST
def document_admin_media_upload(request: HttpRequest, document_id: int):
    actor = cast(User, request.user)
    document = _managed_document(actor, document_id)
    uploaded = request.FILES.get("file")
    if uploaded is None:
        return _media_error(ValidationError({"file": "Choose a file to upload."}))
    try:
        row = upload_document(actor, document, uploaded)
    except ValidationError as exc:
        return _media_error(exc)
    return JsonResponse({"file": admin_file_payload(row)}, status=201)


@enforce_policy("document_admin_media_replace")
@require_POST
def document_admin_media_replace(request: HttpRequest, file_id: int):
    actor = cast(User, request.user)
    row = get_object_or_404(
        DocumentFile.objects.select_related(
            "document_version",
            "document_version__family",
            "document_version__family__owner_office",
        ),
        pk=file_id,
    )
    uploaded = request.FILES.get("file")
    if uploaded is None:
        return _media_error(ValidationError({"file": "Choose a replacement file."}))
    try:
        replacement = replace_file(actor, row, uploaded)
    except ValidationError as exc:
        return _media_error(exc)
    return JsonResponse({"file": admin_file_payload(replacement)})


@enforce_policy("document_admin_media_remove")
@require_POST
def document_admin_media_remove(request: HttpRequest, file_id: int):
    actor = cast(User, request.user)
    row = get_object_or_404(
        DocumentFile.objects.select_related(
            "document_version",
            "document_version__family",
            "document_version__family__owner_office",
        ),
        pk=file_id,
    )
    document_id = row.document_version.pk
    try:
        remove_file(actor, row)
    except ValidationError as exc:
        return _media_error(exc)
    return redirect("document_admin_media", document_id=document_id)


@enforce_policy("document_admin_media_reorder")
@require_POST
def document_admin_media_reorder(request: HttpRequest, document_id: int):
    actor = cast(User, request.user)
    document = _managed_document(actor, document_id)
    raw = request.POST.getlist("order") or request.POST.getlist("ordered_ids")
    ordered_ids = [int(value) for value in raw if str(value).isdigit()]
    try:
        reorder_files(actor, document, ordered_ids)
    except ValidationError as exc:
        return _media_error(exc)
    return redirect("document_admin_media", document_id=document.pk)


@enforce_policy("document_admin_file")
@require_GET
def document_admin_file(request: HttpRequest, file_id: int) -> HttpResponse:
    actor = cast(User, request.user)
    row = get_object_or_404(
        DocumentFile.objects.select_related(
            "document_version",
            "document_version__family",
            "document_version__family__owner_office",
        ),
        pk=file_id,
    )
    assert_admin_readable(actor, row)
    log_event(
        "document.downloaded",
        actor=actor_from_user(actor),
        target=AuditTarget(
            target_type=DocumentFile._meta.label_lower,
            target_id=str(row.pk),
            target_label=row.display_name,
        ),
        metadata={"document_version_id": row.document_version.pk, "admin": True},
    )
    inline = request.GET.get("preview") == "1"
    return stream_file(request, row, as_attachment=not inline)
