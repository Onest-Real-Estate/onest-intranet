  ...
  return (
    ...
    >
      <SelectTrigger
        aria-label={ariaLabel}
        size="sm"
        className={cn("h-8 w-[5.25rem] px-2 font-medium", className)}
      >
        <SelectValue />
      </SelectTrigger>
      <SelectContent position="popper" className="z-[60] max-h-60 min-w-[5.25rem]">
        <SelectGroup>
          {options?.map((option) => (
            <SelectItem
              key={option.value}
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        nav: cn(
          "pointer-events-none absolute inset-x-0 top-0 z-0 flex w-full items-center justify-between gap-1 [&_button]:pointer-events-auto",
        ...
        caption_label: cn(
          ...
          captionLayout === "label"
            ? "text-sm"
            : "text-muted-foreground flex h-8 items-center gap-1 rounded-md pr-1 pl-2 text-sm [&>svg]:size-3.5",
