"""The single audience predicate for announcements.

Every surface that shows an announcement — feed, detail, dashboard,
...
Union semantics
---------------
Selectors are **OR**. A reader who matches *any* selector on an announcement
...
can fan out. The same wording appears in ``docs/announcements.md`` and in the
UI copy.
...
announcements published long before. That is the documented policy, and it is
why nothing here is cached against the announcement.
...
indexed subquery. Answering "can this person see these fifty announcements"
...
def selectors_for(announcement: Announcement) -> QuerySet[AnnouncementAudience]:
    """The stored selectors for one announcement, targets already joined."""
    return AnnouncementAudience.objects.filter(
...
def selector_q(context: AudienceContext) -> Q:
    """The union, expressed over :class:`AnnouncementAudience` rows.
...
def audience_q(context: AudienceContext) -> Q:
    """The union, as one ``Q`` over ``Announcement``.
    ...
    * **It is one indexed subquery** regardless of how many selectors exist,
      which is what keeps the feed off a per-announcement cost curve.
    ...
    if not context.is_authenticated:
        return Q(pk__in=[])
    return Q(
        pk__in=AnnouncementAudience.objects.filter(selector_q(context)).values(
            "announcement_id"
        )
    )


def visible_announcements(user, *, at=None, queryset=None) -> QuerySet[Announcement]:
    """Published, in-window announcements whose audience includes ``user``."""
    moment = at or timezone.now()
    base = Announcement.objects.all() if queryset is None else queryset
    return (
        base.published()
...
def visible_to(user, announcement: Announcement, *, at=None) -> bool:
    ...
    if announcement.pk is None:
        return False
    return visible_announcements(user, at=at).filter(pk=announcement.pk).exists()


def assert_visible(user, announcement: Announcement, *, at=None, reason: str) -> None:
    """``visible_to`` with the denial recorded. Use on direct-object access."""
    if visible_to(user, announcement, at=at):
        return
    log_event(
        ...
        target=AuditTarget(
            ...
            target_id=str(announcement.pk or ""),
            target_label=announcement.slug,
        ),
        outcome=AuditEvent.Outcome.DENIED,
        reason=reason,
    )
    raise PermissionDenied("That announcement is not addressed to you.")


# --------------------------------------------------------------------------- #
# The other direction: who does this announcement reach?
# --------------------------------------------------------------------------- #


def recipients_for(announcement: Announcement, *, at=None) -> QuerySet[User]:
    """Every user the announcement's selectors reach, deduplicated.

    Used by notification fan-out. One queryset with a union filter rather than
    a loop over selectors, so a company-wide announcement is one query and each
    person appears once however many selectors caught them.
    """
    moment = at or timezone.now()
    selectors = list(selectors_for(announcement))
    if not selectors:
...
def _deny(actor, *, reason: str, detail: dict | None = None) -> None:
    log_event(
        "security.announcement.audience_denied",
...
def replace_audience(actor, announcement: Announcement, selectors) -> None:
    """Authorize, then make the stored selectors match exactly what was asked."""
    selectors = list(selectors)
    assert_can_target(actor, selectors)
    before = describe_audience(announcement)
    selectors_for(announcement).delete()
    ...
    log_event(
        ...
        after={"audience": describe_audience(announcement)},
    )
...
def describe_audience(announcement: Announcement) -> list[dict[str, Any]]:
    """camelCase summary of the selectors, in a stable order.

    Ordered by kind then label so two announcements with the same audience
    ...
    rows = [
        ...
        for selector in selectors_for(announcement)
    ]
    return sorted(rows, key=lambda row: (order.get(row["kind"], 9), row["label"]))


def _selector_label(selector: AnnouncementAudience) -> str:
    if selector.kind == Kind.COMPANY:
...
def search_recipients(actor, query: str, *, limit: int = RECIPIENT_SEARCH_LIMIT):
    ...
    if not has_effective_permission(actor, MANAGE_PERMISSION):
        _deny(actor, reason="missing_permission")
        raise PermissionDenied("You cannot address announcements.")
