...
import type { MyTransactionsPageProps, TransactionListRow } from "@/types";
...
export default function MyTransactions() {
  const { items, capabilities } = usePage<MyTransactionsPageProps>().props;
  ...
  function visit(next: Record<string, string>, page?: number) {
    ...
    router.get(
      buildListUrl(routes.my_transactions(), window.location.search, {
    ...
    <PermissionRequired permission={ACCESS}>
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 py-6">
        <PageHeader
          ...
          description="Deals you own or are assigned to."
          actions={
            capabilities.create ? (
              <Button asChild>
          ...
          <SurfaceCardContent className="space-y-4 py-5">
            ...
            {items.items.length === 0 ? (
              <EmptyState
                ...
                description="Open a draft to start tracking a deal."
                actions={
                  capabilities.create ? (
                    <Button asChild>
                      <Link href={routes.transaction_new()}>
...
MyTransactions.layout = (page: React.ReactNode) => <HubLayout>{page}</HubLayout>;
